...
public class ReflectionAnalyzer
{
    ...
    public List<ClassRelationship> ExtractRelationships(List<ClassDiagram> diagrams)
    {
        ...
        foreach (var diagram in diagrams)
        {
            ...
            if (!string.IsNullOrEmpty(diagram.BaseClass))
            {
                ...
                {
                    SourceClass = diagram.ClassName,
                    TargetClass = diagram.BaseClass,
                    Type = RelationshipType.Inherits,
                    SourceNamespace = diagram.Namespace,
                    TargetNamespace = diagram.Namespace
                });
            }
    ...
    public string GenerateRelationshipDiagram(List<ClassDiagram> diagrams)
    {
        var relationships = ExtractRelationships(diagrams);
        ...
        var inheritanceRels = relationships.Where(r => r.Type == RelationshipType.Inherits).ToList();
        var implementationRels = relationships.Where(r => r.Type == RelationshipType.Implements).ToList();

        if (inheritanceRels.Any())
        {
            sb.AppendLine("📊 INHERITANCE HIERARCHY:");
            sb.AppendLine("─────────────────────────");
            foreach (var rel in inheritanceRels)
            {
                sb.AppendLine($"  {rel.SourceClass}");
                sb.AppendLine($"      ↑");
                sb.AppendLine($"      │");
                sb.AppendLine($"  {rel.TargetClass}\n");
            }
        }

        if (implementationRels.Any())
        {
            ...
            foreach (var rel in implementationRels)
            {
                ...
                sb.AppendLine($"      ⇡");
                sb.AppendLine($"      │");
                sb.AppendLine($"  {rel.TargetClass}  (interface)\n");
            }
        }
